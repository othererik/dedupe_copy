...
from dedupe_copy.core import verify_manifest
...
class TestVerify(unittest.TestCase):
    def setUp(self):
        ...
        self.manifest = Manifest(None, save_path=self.manifest_path, temp_directory=self.temp_dir)
        ...
        self.file_list = utils.make_file_tree(
            root=self.temp_dir, file_count=5, file_size=self.file_size, use_unique_files=True
        )
    ...
    def test_verify_missing_file(self):
        ...
        with self.assertLogs('dedupe_copy.core', level='ERROR') as cm:
            ...
            self.assertTrue(any(f"VERIFY FAILED: File not found: {file_to_remove}" in s for s in cm.output))
    ...
    def test_verify_size_mismatch(self):
        ...
        with self.assertLogs('dedupe_copy.core', level='ERROR') as cm:
            result = verify_manifest(self.manifest)
            self.assertFalse(result)
            self.assertTrue(any(f"VERIFY FAILED: Size mismatch for {file_to_modify}: expected {original_size}, got {new_size}" in s for s in cm.output))
